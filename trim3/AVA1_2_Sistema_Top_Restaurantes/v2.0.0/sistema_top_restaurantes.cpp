...
// FUNÇÃO PARA PEGAR OS VALORES DE cod E quantRestaurantes BASEANDO-SE NO ARQUIVO (CHECK)
void getBackup(int *cod, int *quantRestaurantes)
{
	FILE *arquivo;
	...
	// ABERTURA DO ARQUIVO
	arquivo = fopen("lista_restaurantes.txt", "r");
	...
	while (fread(&restaurante, sizeof(CADASTRO), 1, arquivo) == 1)
	{
		...
		// INCREMENTA UM AO CONTADOR REFERENTE AO NÚMERO DE RESTAURANTES CADASTRADOS
...
void adicionar(int *quantRestaurantes, int *cod)
{
	...
	*quantRestaurantes = *quantRestaurantes + 1;
	...
	// ABERTURA DO ARQUIVO
	lista = fopen("lista_restaurantes.txt", "a");
...
void listar(int quantRestaurantes)
{
	...
	{	
		...
		if (opcao != 4)
		{
			...
			// ABERTURA DO ARQUIVO
			lista = fopen("lista_restaurantes.txt", "r");
			...
			switch (opcao)
			{
				...
				case 2: // Listar restaurantes por filtro de nota
					...
					while (fread(&restaurante, sizeof(CADASTRO), 1, lista) == 1)
					{
						// SE A NOTA FOR MAIOR OU IGUAL A MÍNIMA INSERIDA PELO USUÁRIO...
				...
				case 3: // Listar restaurantes por filtro de tipo de comida
					...
					while (fread(&restaurante, sizeof(CADASTRO), 1, lista) == 1)
					{
						// SE O TIPO DE COMIDA DA PESQUISA CORRESPONDER AO TIPO DE COMIDA OBTIDO PELA LEITURA DO ARQUIVO
...
// FUNÇÃO CASO 3 - EXCLUIR UM RESTAURANTE (CHECK)
void excluir(int *quantRestaurantes)
{
	FILE *lista;
	FILE *arq_saida;
	...
	if (*quantRestaurantes > 0)
	{
		...
		if (idExclusao > 0)
		{
			...
			lista = fopen("lista_restaurantes.txt", "r");
			arq_saida = fopen("lista_restaurantes_nova.txt", "w");
			...
			while (fread(&restaurante, sizeof(CADASTRO), 1, lista) == 1)
			{
				// SE O ID DA PESQUISA CORRESPONDER AO ID OBTIDO PELA LEITURA DO ARQUIVO
				if (idExclusao == restaurante.id)
				{
					...
					// CASO SEJA CONFIRMADA, EXECUTA A EXCLUSÃO DO RESTAURANTE
			...
			if (confirmaExclusao == 1)
			{
				...
				status = remove("lista_restaurantes_backup.txt");
				
				// RENOMEIA A LISTA ATUAL COMO BACKUP
				status = rename("lista_restaurantes.txt", "lista_restaurantes_backup.txt");
				...
				status = rename("lista_restaurantes_nova.txt", "lista_restaurantes.txt");
